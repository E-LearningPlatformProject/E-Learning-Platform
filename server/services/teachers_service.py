...
def change_account_info(old: TeacherInfo, new: TeacherInfo):
    ...
    update_query(
        '''UPDATE teachers
        SET first_name = ?, last_name = ?, phone_number = ?, linked_in_account = ?, image = ?
        WHERE id = ? AND is_approved = ?''',
        (merged.first_name, merged.last_name, merged.phone_number, merged.linked_in_account, merged.image, merged.id, merged.is_approved))
